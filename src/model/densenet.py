...
class DenseNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # Use functional relu here instead of mistakenly calling the class
...
def getDenseNet(arch_type):
    if arch_type=='densenet121':
        return DenseNet(num_init_features=64,
                        growth_rate=32,
                        block_config=(6, 12, 24, 16))
    elif arch_type=='densenet161':
        return DenseNet(num_init_features=96,
                         growth_rate=48,
                         block_config=(6, 12, 36, 24))
    elif arch_type=='densenet169':
        return DenseNet(num_init_features=64,
                         growth_rate=32,
                         block_config=(6, 12, 32, 32))
    elif arch_type=='densenet201':
        return DenseNet(num_init_features=64,
                         growth_rate=32,
...
if __name__ == "__main__":
    model = getDenseNet('densenet121')
    print(model)
    x = torch.randn(1, 1, 90, 224, 224)
    y = model(x)
    print(y.shape)